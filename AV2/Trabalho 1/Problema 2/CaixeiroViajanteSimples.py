...
def createNavigationMap(data, dominio):
    ...
    for i in range(data.tail(1).index.item() +1):
        ...
        ax.scatter(data.at[i, 'X'], data.at[i, 'Y'], data.at[i, 'Z'], marker=('*' if i == data.tail(1).index.item() else 'o'), s=(50 if i == data.tail(1).index.item() else 10), linewidth=1, color=('red' if i == data.tail(1).index.item() else 'blue'))
